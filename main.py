...
import torchvision
import datetime
from btp_dataset import BtpDataset, ETHDataset
...
parser.add_argument('--dis_type', default='cnn', choices=['cnn', 'lstm'],
                    help='architecture to be used for discriminator to use')
parser.add_argument('--gen_type', default='lstm', choices=['cnn', 'lstm'],
...
if opt.dataset == "btp":
    dataset = ETHDataset(
        csv_file='/Users/ehsanaghazadeh/PycharmProjects/pythonProject1/ETHUSDT.csv',
        first_data_point_date='2019-01-01',
        last_data_point_date='2020-01-01',
        condition_size=252,
        normalize=True
    )
...
if opt.dis_type == "lstm":
    netD = LSTMDiscriminator(in_dim=48, hidden_dim=256).to(device)
if opt.dis_type == "cnn":
...
if opt.gen_type == "cnn":
    netG = CausalConvGenerator(noise_size=in_dim, output_size=1, n_layers=8, n_channel=10, kernel_size=8,
